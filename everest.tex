...
Eric Agol\altaffilmark{1,2}, Nicholas Saunders\altaffilmark{1}, Rory Barnes\altaffilmark{1,2}}
...
pipeline, \texttt{EVEREST} (EPIC Variability Extraction and Removal for Exoplanet
Science Targets), which regresses exclusively on \emph{local} information.
...
\begin{figure}[hbt]
  \begin{center}
      \includegraphics[width=0.47\textwidth]{figures/short_cad.pdf}
       ...
       short cadence modes. A portion of the raw light curve is displayed at the top,
       and the de-trended light curve is shown in the center. In the bottom panel,
       we plot the down-binned de-trended short cadence light curve (black) and the
       de-trended long cadence light curve (red). Short cadence \texttt{EVEREST 2.0}
...
cadence light curve is ${\sim}30\%$ that of the long cadence light curve.

As we discussed in \ref{sec:impl_shortcad}, one pitfall of our method for de-trending
...
noise. This is the case for very faint stars ($\Kp \lesssim 17$) observed in
short cadence mode, such as EPIC 201831393, which displays visible discontinuities
at many of the breakpoints. {\color{red}More...}
...
\texttt{SysRem} algorithm {\color{red}\citep{Mazeh}}, which identifies and removes signals shared by many
...
\begin{figure}[hbt]
  \begin{center}
      ...
      \caption{Co-trending Basis Vectors (CBVs) for each of the first nine campaigns. 
         ...
         features at the beginning or end of the chunks; the second set of CBVs are
...
GP. We separately apply \texttt{SysRem} to each light curve chunk, obtaining one
co-trending basis vector (CBV) for each chunk of each campaign. We then subtract a linear
fit of this CBV from each light curve and repeat the procedure to obtain a second CBV
for each chunk of each campaign. In order to prevent the CBVs from fitting out or
...
Figure~\ref{fig:cbv}. The first CBV for each chunk of each campaign is plotted in blue,
and the second in red. As expected, the first CBV in each chunk is dominated by a linear trend with S-like
hooks on either end. The second CBVs are predominantly quadratic or cubic.

We fit all de-trended light curves with these CBVs by (ordinary) least squares 
regression. We attempted to regress using the GP of each target, but there is 
typically enough power in the GP model to strongly disfavor a CBV fit. In practice,
our scheme works surprisingly well. Since we only use two CBVs for each chunk, and
since these are dominated by very low order trends, the risk of fitting out true
astrophysical variability is low. Nevertheless, light curves with modulation that
happens to be in phase with the CBVs or stars that dim or brighten on ${\sim}90$ day
timescales could be overfitted by this method. We defer a more careful treatment
of these cases to future work and instead include both the PLD-de-trended flux 
(\texttt{FLUX}) and the CBV-corrected flux (\texttt{FCOR}) in the \texttt{EVEREST 2.0}
FITS files.
...
\begin{figure}[hbt]
  \begin{center}
      \includegraphics[width=0.47\textwidth]{figures/201345483.pdf}
       \caption{EPIC 201345483, a $\Kp = 15$ campaign 1 {\color{red} planet host} de-trended
       with \texttt{K2SFF} (top) and \texttt{EVEREST 2.0} (bottom). The CDPP of each light curve
       is indicated in the top left. The folded transit is shown at right. The \texttt{EVEREST 2.0}
...
\begin{figure}[hbt]
  \begin{center}
      \includegraphics[width=0.47\textwidth]{figures/201862715.pdf}
       \caption{EPIC 201862715, a saturated campaign 1 {\color{red} planet host}. As in 
...
shows EPIC 201345483, a faint $\Kp = 15$ {\color{red} planet host}.
...
{\color{red} planet host}, EPIC 201862715. With the column-collapsing scheme and the
...
(dampened oscillation amplitudes, {\color{red}loss of coherence in the astrophysical signal}, 
...
\label{sec:crowded}
{\color{red} I still need to run tests on a sample of crowded stars to see how the new
version does. Ethan, do you have a list of planet candidate hosts that \texttt{EVEREST 1.0}
screwed up? That will help me track down the crowded stars.}
...
{\color{red}\url{http://dx.doi.org/XX.XXX/XXXXX}}. The reader is encouraged to use this code
