...
\note{Despite significant differences in their implementation, all of these pipelines
...
L2 regularization, since it has an analytic solution. L2 regularization is mathematically equivalent to
...
\note{The principal difference between PLD and other methods used to correct $K2$ light curves
...
derived from the behavior of many stars across the detector, to accomplish the same thing. 
...
\note{A simple way of implementing this would be to include the two \emph{Kepler} motion vectors (the
...
vectors of a set of other targets located on the same CCD module in the regression; we dub this method \texttt{nPLD}, for \emph{n}eighboring
...
\begin{figure}[hbt]
  \begin{center}
      \includegraphics[width=0.45\textwidth]{figures/saturated_star.pdf}
       ...
       \note{Re-run script for axis labels.}}
...
in the \texttt{EVEREST 2.0} light curve. As before, \texttt{EVEREST 2.0} correctly de-trends the light curve with
...
\subsection{Misc}
\note{EPIC 220651426: different de-trending power in each segment. CBV tails.}
...
\end{center}
%
Data flagged with bit \texttt{23}

Data flagged with bit \texttt{24} is missing in the original TPF and can thus be safely
ignored.

While most of the data points flagged with bit \texttt{25} are poorly de-trended
instrumental outliers, exoplanet transits may also be flagged with this bit

The four remaining extensions include the PLD regressors, the target aperture, and additional
information used internally by the Python code.
...
\end{lstlisting}
%
or directly from source on the project \texttt{github} page. The primary way of interfacing
with the catalog is to instantiate an \texttt{Everest} object. The following
...
\note{Some discussion of how to deal with remaining outliers/flagged cadences.}
...
%\note{\url{http://dx.doi.org/XX.XXX/XXXXX}}. 
%
%
